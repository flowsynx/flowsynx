...
internal class StorageService : IStorageService
{
        ...
        StorageSearchOptions searchOptions, StorageCopyOptions copyOptions, CancellationToken cancellationToken = default)
    {
        ...
        {
            bool isFile;
            if ((isFile = StorageHelper.IsFile(sourceStorageNormsInfo.Path)) != StorageHelper.IsFile(destinationStorageNormsInfo.Path))
                throw new StorageException("The specified destination-path is of a different type than the source-path.");
    ...
    {
        if (!StorageHelper.IsRootPath(destinationDirectory))
    ...
    public async Task Move(StorageNormsInfo sourceStorageNormsInfo, StorageNormsInfo destinationStorageNormsInfo, StorageSearchOptions searchOptions, StorageMoveOptions moveOptions, CancellationToken cancellationToken = default)
    {
        try
        {
            ...
            {
                throw new StorageException("The source and destination path are identical and overlap.");
            }
            ...
            if (!StorageHelper.IsRootPath(sourceStorageNormsInfo.Path))
