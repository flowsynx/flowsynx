...
internal class DateParser : IDateParser
{
    ...
    protected DateTime ParseDateTimeWithSuffix(string dateTime)
    {
        if (!HasSuffix(dateTime))
        {
            ...
            throw new DateParserException(FlowSyncCoreResource.FileSystemDateParserInvalidInput);
        }
    ...
    protected double ExtractValue(string dateTime, string key, ref int position)
    {
        ...
        if (charLocation < 0)
        {
            ...
            throw new DateParserException(string.Format(FlowSyncCoreResource.FileSystemDateParserCannotExtractValue, dateTime));
        }
