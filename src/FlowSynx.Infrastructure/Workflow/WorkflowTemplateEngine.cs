...
public class WorkflowTemplateEngine
{
    ...
    private readonly IDictionary<string, IWorkflowFunction> _functions;
    private readonly IDictionary<string, object?> _results;

    public WorkflowTemplateEngine(JObject variables)
    {
        _variables = variables;
        _functions = new Dictionary<string, IWorkflowFunction>();
        _results = new Dictionary<string, object?>();
    }

    public void RegisterFunction(string name, IWorkflowFunction function)
    {
    ...
    private object ApplyTransformation(object? value, IWorkflowFunction function, List<object> arguments)
    {
        function.ValidateArguments(arguments);
        return function.Execute(value, arguments);
    }
