...
public class GeneValidator : IGeneValidator
{
        ...
        CancellationToken cancellationToken)
    {
        ...
        foreach (var paramDef in blueprint.NucleotideSequences)
        {
            if (paramDef.Required && !instance.NucleotideSequences.ContainsKey(paramDef.Name))
            {
                errors.Add($"Required parameter '{paramDef.Name}' is missing");
            }

            if (instance.NucleotideSequences.TryGetValue(paramDef.Name, out var value))
            {
        ...
        foreach (var sequence in instance.NucleotideSequences.Keys)
        {
            if (!blueprint.NucleotideSequences.Any(p => p.Name == sequence))
            {
                errors.Add($"NucleotideSequence '{sequence}' is not defined in the gene blueprint");
            }
        }
        ...
        CancellationToken cancellationToken)
    {
        ...
        {
            foreach (var network in gene.RegulatoryNetwork)
            {
                if (!geneIds.Contains(network.Value))
                {
                    errors.Add($"Gene '{gene.Id}' regulatoryNetworks on non-existent gene '{network}'");
                }
            }
        }
        ...
        for (int i = 0; i < chromosome.Genes.Count; i++)
        {
            for (int j = i + 1; j < chromosome.Genes.Count; j++)
            {
                ...
                if (blueprint1?.EpistaticInteraction?.IncompatibleGenes?.Contains(gene2.GeneBlueprintId.Value) == true ||
                    blueprint2?.EpistaticInteraction?.IncompatibleGenes?.Contains(gene1.GeneBlueprintId.Value) == true)
                {
