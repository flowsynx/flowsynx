...
internal class ExecuteWorkflowHandler : IActionHandler<ExecuteWorkflowRequest, Result<ExecutionResponse>>
{
    ...
    public async Task<Result<ExecutionResponse>> Handle(ExecuteWorkflowRequest request, CancellationToken cancellationToken)
    {
        try
        {
            ...
            var deserializedJson = _deserializer.Deserialize<ExecuteWorkflowRequestDefinition>(jsonString);

            var result = await _workflowExecutionService.ExecuteWorkflowAsync(
